...
export default function CreateDailyLogPage() {
  ...
  const handleSubmit = async (formData: any) => {
    try {
      ...
      if (isPhotoLoading) {
        setSubmitError('Vui lòng đợi upload ảnh hoàn tất trước khi tạo nhật ký');
        setIsSubmitting(false);
        return;
      }
      
      // Prepare photos from pending photos
      const photosToSubmit = pendingPhotos.map((photo, index) => {
      ...
      if (response.ok) {
        ...
        // Navigate to the created daily log detail page
        router.push(`/dashboard/projects/${projectId}/daily-logs/${dailyLogData.id}`);
      } else {
        ...
        throw new Error(errorData.error?.message || `Failed to create daily log: ${response.status}`);
      }
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'An error occurred';
  ...
  const handlePhotoDelete = async (photoId: string) => {
    try {
      ...
      setPendingPhotos(prev => prev.filter((_, index) => index.toString() !== photoId));
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Unknown error';
      ...
      <div className="container mx-auto p-6 space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Không thể tải thông tin dự án</p>
          </div>
        </div>
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Tạo nhật ký thi công mới</h1>
      ...
      {!submitError && pendingPhotos.length > 0 && (
        <Alert className="border-green-200 bg-green-50 dark:bg-green-950/20">
          ...
          <AlertDescription className="text-green-800 dark:text-green-200">
            Đã upload {pendingPhotos.length} ảnh thành công. Bạn có thể tiếp tục tạo nhật ký.
          </AlertDescription>
        </Alert>
      )}
      ...
      <Card className="shadow-sm">
        <CardHeader className="border-b bg-gray-50/50 dark:bg-gray-900/50">
          <CardTitle className="flex items-center gap-2 text-xl">
            ...
            Thông tin nhật ký thi công
          </CardTitle>
          <CardDescription className="text-base">
            Điền đầy đủ thông tin về công việc thi công trong ngày. Tất cả các trường có dấu (*) là bắt buộc.
          </CardDescription>
        </CardHeader>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="font-semibold text-amber-900 dark:text-amber-100 text-sm">
                Mẹo tạo nhật ký hiệu quả
              </h4>
              <ul className="text-amber-700 dark:text-amber-300 text-xs mt-1 space-y-1">
                <li>• Ghi chú chi tiết về công việc đã thực hiện trong ngày</li>
                <li>• Chụp ảnh minh chứng cho từng hạng mục công việc</li>
                <li>• Ghi nhận các vấn đề phát sinh để xử lý kịp thời</li>
                <li>• Cập nhật số lượng công nhân và giờ làm việc chính xác</li>
              </ul>
            </div>
          </div>
        </CardContent>
